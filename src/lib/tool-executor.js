...
async function handleBomYukleOnizleme({ dosyaAdi, projeAdi }, yuklenenDosyalar) {
  ...
  try {
    ...
    const kayitlar = []; // { ogn, parcaNo, tanim, miktar, en, boy, yukseklik, cap, agirlik, malzeme, seviye, parentOgn }
    ...
    return {
      ...
      cacheKey,
      mesaj: `${istatistik.toplam} satır okundu. ${istatistik.montaj} montaj, ${istatistik.parca} parça. Onaylarsan ${projeAdi} projesine yazacağım.`,
    };
  } catch (err) {
...
async function handleBomYukleOnayla({ dosyaAdi, projeAdi }, yuklenenDosyalar) {
  ...
  for (const k of kayitlar) {
    ...
    if (k.tanim) alanlar["Tan\u0131m"] = k.tanim;
