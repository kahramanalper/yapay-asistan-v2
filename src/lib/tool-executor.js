...
// Tablo adı çözümleme (kullanıcı farklı yazabilir)
...
async function executeTool(toolName, toolInput) {
  try {
    switch (toolName) {
      case "kayit_listele":
        return await handleList(toolInput);
      case "kayit_olustur":
        return await handleCreate(toolInput);
      case "kayit_guncelle":
        return await handleUpdate(toolInput);
      case "kayit_sil":
        return await handleDelete(toolInput);
      case "parca_nerede":
        return await handleParcaNerede(toolInput);
      case "isleme_al":
        return await handleIslemeAl(toolInput);
      case "durum_degistir":
        return await handleDurumDegistir(toolInput);
      default:
...
async function handleIslemeAl({ parcaNo, projeAdi }) {
  ...
  results.adimlar.push("BOM durumu güncellendi: Satın Almada");

  // 3. Hammadde (HM-) kaydı BOM'a ekle
  const hmParcaNo = `HM-${parcaNo}`;
  const hmBom = await createRecord("BOM", {
    "Par\u00e7a No": hmParcaNo,
  ...
  results.adimlar.push(`Hammadde kaydı eklendi: ${hmParcaNo}`);

  // 4. Hammadde SA'ya ekle
  const saRecord = await createRecord("Sat\u0131n Alma", {
    "Par\u00e7a No": hmParcaNo,
    "Tan\u0131m": `${bomRecord["Tan\u0131m"] || parcaNo} - Hammadde`,
    Miktar: bomRecord.Miktar || 1,
    Durum: "Bekliyor",
  ...
  results.adimlar.push("SA kaydı oluşturuldu: Bekliyor");

  // 5. İmalat kaydı aç
  // Parça tipini prefix'ten belirle
  let asama = "Torna";
  if (parcaNo.startsWith("F-")) asama = "Freze";
  else if (parcaNo.startsWith("L-")) asama = "Lazer";
  else if (parcaNo.startsWith("K-")) asama = "Kaynak";

  const imalatRecord = await createRecord("\u0130malat", {
    "Par\u00e7a No": parcaNo,
    "A\u015fama": asama,
    Durum: "Hammadde Bekliyor",
    "Proje Ad\u0131": projeAdi,
  });
  results.adimlar.push(`İmalat kaydı açıldı: Hammadde Bekliyor (${asama})`);
...
async function handleDurumDegistir({ tablo, parcaNo, projeAdi, yeniDurum, ekBilgi }) {
  ...
  const results = { adimlar: [`${tableName} durumu güncellendi: ${yeniDurum}`] };

  // --- ZİNCİRLEME ETKİLER ---

  // SA "Teslim Alındı" → KK aç, BOM güncelle, İmalat tetikle
  if (tableName === "Sat\u0131n Alma" && yeniDurum === "Teslim Al\u0131nd\u0131") {
    ...
    results.adimlar.push("KK kaydı oluşturuldu: Bekliyor");
    ...
    if (bomRecs.length > 0) {
      ...
      results.adimlar.push("BOM durumu güncellendi: Teslim Alındı");
    }

    // Hammadde ise İmalat tetikle
    if (parcaNo.startsWith("HM-")) {
      ...
      if (imalatRecs.length > 0) {
        ...
        results.adimlar.push(`İmalat güncellendi: ${anaParca} Devam Ediyor`);
      }
    }
  }

  // İmalat "Tamamlandı" → KK aç
  ...
  if (tableName === "Kalite Kontrol" && yeniDurum === "Onay") {
    ...
    if (bomRecs2.length > 0) {
      ...
      results.adimlar.push("BOM durumu güncellendi: Depoda");
    }
  }
  ...
  if (tableName === "Kalite Kontrol" && yeniDurum === "Red") {
    ...
    results.adimlar.push("⚠️ Parça reddedildi — yeniden üretim/tedarik gerekli");
  }
